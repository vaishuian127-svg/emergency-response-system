...
if "connected" not in st.session_state:
    ...
    if cookie_manager:
        try:
            saved = cookie_manager.get(cookie="user_info")
            if saved:
    ...
    with col:
        ...
        if st.button("🚑 Login", key="login_btn"):
            ...
            if username in valid_users and valid_users[username] == password:
                ...
                if cookie_manager:
                    try:
                        cookie_manager.set("user_info", json.dumps({"name": username, "picture": ""}))
                    except Exception:
                        pass
                st.rerun()
            else:
...
def show_app():
    ...
    <style>
        ...
        .user-badge {{
            background: linear-gradient(135deg, #ff8800 0%, #ff3300 50%, #ff8800 100%) !important;
            ...
            font-weight: 700 !important;
            box-shadow: 0 4px 15px rgba(255, 51, 0, 0.4) !important;
            transition: all 0.4s ease-in-out !important;
        }}
        .user-badge:hover {{
            border-color: #ffffff !important;
            box-shadow: 0 8px 25px rgba(255, 51, 0, 0.65) !important;
        ...
        button[aria-label*="Logout"]:hover {{
            border-color: #ffffff !important;
            box-shadow: 0 8px 25px rgba(255, 68, 68, 0.65) !important;
            background-position: right center !important;
            transform: scale(1.05) translateY(-2px) !important;
        }}
    </style>
    ...
    with tb2:
        if user_picture:
            ...
            st.markdown(f'<div style="display: flex; justify-content: center; margin-top: 6px;"><div class="user-badge"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" style="margin-right: 6px; vertical-align: middle;"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>{user_name}</div></div>',
                        unsafe_allow_html=True)
    with tb3:
